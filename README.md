...
Copy `.env.example` to `.env.local`. Public browser configuration may use `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, and `VITE_STRIPE_CHECKOUT_URL`. Keep `STRIPE_SECRET_KEY` and any Supabase service-role key server-side in Cloudflare secrets only.
