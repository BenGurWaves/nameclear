...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const tickerDirectory = await tickerResponse.json() as Record<string, SecTicker>;
  const wanted = normalize(query);
  const companies = Object.values(tickerDirectory);
  const match = companies.find((company) => normalize(company.ticker) === wanted) ?? companies.find((company) => normalize(company.title).includes(wanted));
  ...
  const filings = recent.form.map((form, index) => {
    ...
    return { ticker: match.ticker, company: match.title, type: form === "4" ? "Form 4" : form.replace("SCHEDULE ", ""), date: recent.filingDate[index], time: "", title: recent.primaryDocDescription[index] || form, detail: filingDescription(form), href: `https://www.sec.gov/Archives/edgar/data/${match.cik_str}/${accessionPath}/${primaryDocument}`, tone: form === "8-K" ? "lime" : form === "4" ? "orange" : form.includes("13") ? "blue" : "purple" };
  }).filter((_, index) => trackedForms.has(recent.form[index])).slice(0, 20);
  return Response.json({ ticker: match.ticker, company: match.title, cik, filings }, { headers: { "Cache-Control": "public, max-age=60" } });
};
